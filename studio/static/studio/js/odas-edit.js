...
$('.oda-materias li').each(function(){
	let subjectName = $(this).text().replace(/\s/g,"-");
	let classQuantity = $('.'+ subjectName).length;
...
$('.oda-tags-list li').each(function(){
	let tagName = $(this).text().replace(/\s/g,"-");
	$(this).attr({
...
$('.oda-materias li').each(function(){
	let matName =$(this).ignore("span").text().replace(/\s/g,"-");
	$(this).attr({
		"data-control-type":"button-filter",
		"data-control-action":"filter",
        "data-selected":" false",
